...
export default async function AdminTemplatePage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const [templates, fullDict, versions] = await Promise.all([
    listTemplates(),
    getDictionary(lang),
    fetchAvailableAppVersions(),
  ]);
  const dict = fullDict.admin.template;

  return (
    <div className="flex min-h-screen flex-col">
      <SiteHeader lang={lang as Locale} nav={fullDict.nav} />
      <main className="container mx-auto flex max-w-6xl flex-1 flex-col gap-6 px-4 py-12">
